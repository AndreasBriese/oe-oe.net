...
// globals
    var input, output, eingaben, chooser, sKey, lKey, audio, audioPlay, userNick,
        ...
        longMessage, msgLen, multiFrame = false,
        storedMssg = [],
        kpflen = 3,
        outMssgNum = 0,
        inMssgNum = 0;
...
// draw the connect-Info Image
    drawConnectInfo = function(txt){
        ...
        for(var i=0;i<txt.length;i++){
            ctx.fillText(txt[i], i*12+5*Math.random(), 1+8*Math.random());
        }
    ...
    function infoOverlay(content){
        ...
        oo.innerHTML += '<br/><span style="font-size:90%;">Bitte kopieren und dem Chatpartner mitteilen,</span><br/><span style="font-size:90%;"> als Bild kopieren &amp; versenden</span><br/><span style="font-size:90%;">oder im <button id="buttnChatLocker">chatlocker</button> ablegen</span><div style="margin-top:4px;border:1px solid green;text-weight:700;padding:2px;margin-left:10px">Achtung! Wenn Du das Fenster schliesst, wird Dein Kanal geschlossen!</div>';
        ...
        $("buttnChatLocker").onclick = lockerPopUp;
        ta.value = content;
        ta.focus();
        ta.select();
        window.scrollTo(0, 0);
        ...
        if (typeof(e.data)==="string") {
            ...
            reader.addEventListener("loadend", function(){
                ...
                if(type==="canvas/png"){
                    ...
                    // secret Image overkill
                    setTimeout('try{$("vorhang").parentNode.removeChild($("vorhang"))}catch(e){}', 1500);
                    delay(killSecretImage, 1200, ctx, ctxdata, 0);
                    // secretImage display
                    body.appendChild(vorhang);
                }else{
    ...
    function onClose() {
        ...
        try{$("infoOverlay").parentNode.removeChild($("infoOverlay"));}catch(e){}
    }
    ...
    var lockerPopUp = function(){
        ...
        popUp.id = "popUp";
        popUp.style.top = (output.lastChild.offsetTop + ((navigator.mobile)?1.1:2.2)* output.lastChild.offsetHeight) + 50+ "px" ;
        popUp.style.left = "40px" ;
        popUp.innerHTML = 'Dein Schluesselsatz (<a href="https://xn----0gab.net/">?</a>):<input id="pphrase" type="text" value="30+ Zeichen <return>"></input>';
        body.appendChild(popUp);
        var pphrase = $('pphrase');
        pphrase.onfocus = function(){this.value="";};
        pphrase.onkeypress = function(e){
            var pp = this.value;
            if (pp.length>29) e.target.style.backgroundColor="rgba(0,220,0,0.9)";
    ...
    var nickPopUP = function(content){
    ...
    function prepareSecretImage(url, file) {
        var fle = file;
        if (!!fle){
            if(multiFrame){
                ...
                // sendMessage();
                delay(sendSecretImage, 100, url, fle);
            }
        }
    }
    ...
    function send2locker(passPhrase){
...
//             var prog = $tag("progress")[0];
//             prog.parentNode.removeChild(prog);
        }
    };
    
    killSecretImage = function(ctx, imgdata, zle){
/**/
    };

    sendSecretImage = function(url, files){
/**/
    };
    
    secretImage = function(readerResult, maxW, maxH, func){
/**/
    };
    ...
    var prompt = function(){
        if($("prompt")){
            document.body.removeChild($("prompt").parentNode);
        }
        var prompt = document.createElement("div");
        prompt.innerHTML = '<div id="prompt" class="promptHide"><table width="100%"><tr><td id="choosYes">SecretImage*</td><td id="choosPrompt"></td><td id="choosNo">Datei</td></tr></table></div>';
        var a = arguments;
        a[0].appendChild(prompt);
        var yPos = a[1], 
            text = a[2], 
            yesCallback = a[3], 
            yesParam = a[4], 
            noCallback = a[5], 
            noParam = a[6];
        $("choosPrompt").innerHTML = text;
        $("prompt").className = "promptShow";
        $("prompt").style.top = footer.offsetTop-120 +"px";
        $("prompt").style.zIndex = "155";
        if(window.ontouchstart){
            $("choosYes").ontouchstart = function(){yesCallback.applier(yesParam);}
            $("choosNo").ontouchstart = function(){noCallback.applier(noParam);}
            $("choosPrompt").ontouchstart = noop;
            $("prompt").addEventListener("touchstart", function(){this.className="promptHide"}, true);
        }else{
            $("choosYes").onclick = function(){yesCallback.applier(yesParam);}
            $("choosNo").onclick = function(){noCallback.applier(noParam);}
            $("choosPrompt").onclick = noop;
            $("prompt").addEventListener("click", function(){this.className="promptHide"}, true);
        }  
    };
    ...
    function init() {
        ...
        if(navigator.mobile && navigator.userAgent.match(/(ipad|ipod|iphone)/i) && navigator.info[1]<7){
            ...
            fileUploader.onchange = function(e) {
                if(this.files.length==1 && this.files[0].type.match(/image/) && this.files[0].type.match(/(jpg|jpeg|gif|png)/i)){
                    prompt.applier([body, 0, 'Als SecretImage* senden? <br/><p style="font-size:70%>"Upload abbrechen durch Schliessen des Fensters.<br/>(* SecretImage wird beim Empfaenger nach 1s geloescht.<br/><span style="color:#700">Man kann SecretImages screenshotten!</span>))</p> Klick hier, um den Upload abzubrechen.', prepareSecretImage, ['https://{{.XHR}}', this.files[0]], prepareUploadFile, ['https://{{.XHR}}', this.files[0]]]);
                }else{
